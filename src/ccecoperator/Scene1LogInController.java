...
public class Scene1LogInController extends ControllerClass {
    ...
    @FXML
    private PasswordField passwordManager;
    
    @FXML
    private TextField usernameEmployee;
    ...
    Path pathClient = Paths.get("userOfEmployee.txt"); //userOfEmployee.txt

    Path pathAd = Paths.get("userOfManager.txt"); //userOfManager.txt
    ...
    @FXML
    private void handleButtonLogInManagerAction(ActionEvent event) {

        try {
            if (usernameManager.getText().equalsIgnoreCase("") || passwordManager.getText().equalsIgnoreCase("")) {
                labelLogIn.setText("Please enter username and password");//if 1 of two are fields empty
            } else {
                boolean userNameExistance = false;
                ...
                loginTemp[0] = usernameManager.getText();

                loginTemp[1] = passwordManager.getText();
                ...
                if (Files.exists(pathAd)){
                    ...
                    for (int i = 0; i < tempLoad.size(); i = i + 2) {//going thrue usernames
                        if (loginTemp[0].equalsIgnoreCase(tempLoad.get(i)) && loginTemp[1].equalsIgnoreCase(tempLoad.get(i + 1))) {
                            //check username and pass matching

                            System.out.println("Username and password is correct!");
                            
                            DataStorage.getInstance().setIsAdmin(true);
    ...
    @FXML
    private void handleButtonLogInEmployeeAction(ActionEvent event) {

        try {
            if (usernameEmployee.getText().equalsIgnoreCase("") || passwordEmployee.getText().equalsIgnoreCase("")) {
                labelLogIn.setText("Please enter username and password");//if 1 of two are fields empty
            } else {
                boolean userNameExistance = false;
                ...
                loginTemp[0] = usernameEmployee.getText();

                loginTemp[1] = passwordEmployee.getText();

                if (Files.exists(pathClient)){
                    ArrayList<String> tempLoad = (ArrayList<String>) Files.readAllLines(pathClient);//load
                    ...
                    for (int i = 0; i < tempLoad.size(); i = i + 2) {//going thrue usernames
                        if (loginTemp[0].equalsIgnoreCase(tempLoad.get(i)) && loginTemp[1].equalsIgnoreCase(tempLoad.get(i + 1))) {
                            ...
                            DataStorage.getInstance().setIsAdmin(false);
